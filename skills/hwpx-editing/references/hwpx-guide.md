...
13. **다중 run 문단 복제 시 run 구조 뭉갬 → 서식 불균일.** 참고문헌처럼 run마다 charPr이 다른 문단은 `run[1:]` 제거 후 전체 텍스트를 run[0]에 넣으면 **run[0] 서식이 문단 전체에 먹는다**. run 단위로 채운다(§3).
14. **문단 복제 템플릿으로 첫 문단 사용 → secPr 중복 + 텍스트 run 소실.** 섹션 첫 `<hp:p>`가 `secPr`를 품는다. 템플릿은 **secPr 없는 문단**에서 고른다(§3).
...
- **다중 run 문단을 복제할 때 run 구조를 뭉개지 말 것.** 참고문헌·강조가 섞인 문단은 한 문단이 여러 run으로 나뉘고 **run마다 charPr이 다르다**(예: 저자 bold / 제목 plain / 저널명 italic). 흔한 복제 관용구인 "`run[1:]` 제거 후 `t[0].text`에 전체 텍스트 주입"을 쓰면 **서식이 통째로 무너지고 run[0]의 서식이 문단 전체에 먹는다.** 새로 넣은 항목만 전부 굵게 나오는 식이라 구조검증으로는 안 잡히고 눈으로만 보인다.
  ```python
  # 원본의 run 패턴을 먼저 읽는다
  pat = [r.get('charPrIDRef') for r in tpl.findall(f'.//{P}run')]   # 예: ['46','47','48','47']
  # 텍스트를 조각으로 받아 run 단위로 채운다 (한 덩어리로 넣지 않는다)
  parts = [('46', 저자연도), ('47', ' '+제목+' '), ('48', 저널), ('47', 나머지)]
  ```
  조각 경계의 **공백도 원본을 따라간다** — 저널명 앞 공백을 빼먹으면 `studies.BMJ Open`처럼 붙는다.
  빌드 후 **모든 항목의 run 패턴이 같은지 단정문으로 확인**한다(항목마다 패턴을 뽑아 비교).
