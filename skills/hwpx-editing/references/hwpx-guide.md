...
25. **lxml 원소를 `id()`로 식별 → 집합 멤버십이 «전부» 어긋난다.** `{id(e) for e in root.iter(...)}` 로 만든 집합을 «다른 순회»에서 조회하면 거의 다 빗나간다 — lxml 원소는 순회마다 새로 만들어지는 **프록시**라 `id()`가 안정적이지 않다. 실측(2026-09-05): 표 안팎을 가르려고 그렇게 했다가 **401개 중 387개가 «바깥»으로 오분류**됐고, 표 셀에 본문 서식이 먹었다. **예외도 경고도 없고 XML 검사도 통과한다.** 조상·자손 관계는 `el.iterancestors(TAG)`·`el.iter(TAG)`로 «구조»를 물어서 판정한다(멤버십 집합을 쓰려면 `id()` 대신 원소 자체를 담되, 같은 트리를 다시 파싱하지 않을 때만).
...
**발송본은 «한글이 조판까지 끝내고 저장한 것»으로 낸다.** 구조를 편집하면 `linesegarray`를
지우게 되는데(§3), 그 상태로 내보내면 받는 쪽에서 파일을 뜯었을 때 「사람이 한글에서 저장한
게 아니다」가 바로 보인다. 기능상 문제는 없지만(한글이 열 때 다시 계산한다) 남에게 보내는
문서라면 지우고 갈 이유가 없다. 실측(2026-08-03):

- 한글은 **있는 캐시는 보존하지만 없으면 만들지 않는다** — 그냥 열고 저장하면 `0 → 0`.
- **조판을 끝까지 시킨 뒤** 저장해야 생긴다(`0 → 1,696`). 쪽수를 읽는 것이 가장 확실하다
  (페이지를 다 세려면 조판이 끝나야 한다). PDF 저장·`MoveDocEnd`+대기도 된다.
- ⚠️ **비동기라 한 번에 안 될 때가 있다.** 같은 코드가 1,696을 냈다가 0을 냈다 →
  **결과를 세어 확인하고 재시도**한다.
- ⚠️ 열려 있는 **그 경로로 덮어쓰면 반영되지 않는다** → 새 경로 저장 후 바꿔치기.

→ **`tools/hwp_render.py`의 `save_paginated(src, out, also=[(path,"HWP")])`** 가 넷을 다 한다.
`.hwp` 발송본도 같은 호출로 함께 낸다. 낸 뒤에는 **두 파일을 각각 렌더해 원본 렌더와
텍스트를 대조**하라 — 형식 변환으로 무엇이 사라졌는지는 렌더 대조로만 보인다.
필드가 «필드로» 살아남았는지는 한 걸음 더 가서 **왕복 변환**(hwp → hwpx) 후
`crossref_check.py`로 확인한다(렌더가 같다는 것은 «지금 보이는 번호»가 같다는 뜻일 뿐이다).

⚠️ **렌더 텍스트로 본문을 대조할 때 메모(MEMO) 풍선이 본문 흐름에 끼어든다.** PDF 추출에서
문단 중간에 작성자·날짜·메모 내용이 삽입돼 문장이 끊긴 것처럼 보인다 — 대조기가 「불일치」로
띄우면 그 자리부터 의심하라. 실제로 2건이 이 오탐이었다.
